...
The threat model below was originally written against the spec alone. Two reference implementations now exist and have been re-walked against every scenario:

- **`@orangecheck/pledge-core`** (TypeScript, npm) — full envelope-only verify path: shape + version + canonical-message reconstruction + id check + BIP-322 signature verify (with §7.3 agent-address override) + §4.4 state classification.
- **`orangecheck.pledge`** (Python sub-module of the `orangecheck` PyPI package) — same surface as the TypeScript SDK; cross-impl conformance against every vector in `test-vectors/` is enforced in CI.

Both SDKs cover **SPEC §9.1 steps 1–7 plus step 9** as of `@orangecheck/pledge-core@0.2.0` / `orangecheck@0.3.0`:

- **Steps 1–4** (version + shape + canonical-message reconstruction + BIP-322): in-SDK, no caller hooks needed.
- **Step 5 — bond live re-resolution against Bitcoin chain state.** SDK ships `verifyBond` / `verify_bond` as a pure algorithm with a caller-injected attestation lookup. The hosted `/verify` and `/p/<id>` surfaces on pledge.ochk.io supply this hook via `@orangecheck/sdk`'s `check()`, which does Nostr-discovery + Esplora chain query + signature verification, all browser-side. Cache TTL discipline remains the caller's.
- **Step 7 — agent §7.3 delegation chain.** When `via_delegation` is set AND a `delegationLookup` adapter is supplied, the SDK runs §7.3 steps 1–5 (resolve delegation envelope, confirm principal == swearer, agent == agent_address, scope constraints fit the pledge, expires_at > sworn_at). The hosted surfaces wire this via `@orangecheck/agent-core`'s `verifyDelegation`. Without the hook, the agent path falls back to step-6-only (signature against agent_address).
- **Step 9** (state classification): in-SDK, pure function.

What's still consumer-side after this revision:

- **Step 8 — deterministic mechanism re-evaluation against public state.** The seven `resolution.mechanism` grammars are validated at the regex level (`validate_resolution_query`); actual chain / Nostr / HTTP / DNS / Vote evaluation against the predicate is left to consumers. The hosted surfaces don't run step 8 themselves; they rely on a published outcome envelope being on Nostr (which the §4.4 state machine then incorporates) for the resolved verdict to surface.
- **`http_get_hash` majority-of-three and `dns_record` multi-resolver agreement** (SPEC §3.4.5, §3.4.6). The SDKs validate query shape only; consumers ship the resolver discipline at outcome-publication time.

Cross-impl conformance is gated in CI (`oc-packages/.github/workflows/packages.yml::conformance`): every PR runs both SDKs against the live `oc-pledge-protocol/test-vectors/` and fails if either drifts. As of this revision, all 28 vectors pass on both SDKs.
...
15. **Agent goes rogue under unscoped delegation.** A principal issues an OC Agent delegation with `pledge:create` scope and no constraints; the agent swears pledges the principal didn't intend. *Mitigated by scope grammar*: SPEC §7.3 supports `max_bond_sats`, `mechanism`, and `counterparty` constraints. Principals SHOULD issue narrowly-scoped delegations. *Residual risk accepted*: a principal who issues an unconstrained delegation accepted the risk; the public ledger reflects that the principal is the swearer of record, with the delegation cited. **Implementation note (2026-05)**: as of `@orangecheck/pledge-core@0.2.0` / `orangecheck@0.3.0`, `verifyPledge` accepts a `delegationLookup` hook that runs SPEC §7.3 steps 1–5 (resolve delegation + confirm principal/agent + validate `pledge:create` scope constraints + temporal). The hosted `/verify` and `/p/<id>` surfaces on pledge.ochk.io supply this hook automatically using `@orangecheck/agent-core`'s `verifyDelegation`, so agent-delegated pledges fail with `E_DELEGATION_*` codes when scope or principal/agent or expiry checks fail. SDK consumers must wire the hook themselves; a single-file Nostr-based adapter is documented in `oc-pledge-web/src/lib/orangecheck-delegation.ts`. Without the hook, the agent path falls back to envelope-shape + signature-only checks (step 6 only).

16. **Delegation forgery.** An attacker fabricates a delegation that authorizes them as agent for a target principal. *Mitigated*: the delegation is BIP-322-signed by the principal under OC Agent v1's verification rules. Fabricated delegations fail principal-signature check. **Implementation note (2026-05)**: same as scenario 15 — pledge-core 0.2.0+ runs the principal-signature check via the `delegationLookup` hook when supplied. The pledge envelope itself is sound (the agent_address claim is unsigned, but the BIP-322 sig.value verifies under agent_address — so a fabricated agent_address would also need a matching agent private key to produce a valid sig). The forgery vector exists only if a consumer skips the delegation lookup; the hosted web surfaces don't.
...
23. **URL-id mismatch via relay manipulation.** A pledge resolver (`/p/<id>` or any consumer building a similar surface) fetches a Nostr event for a given `<id>` URL slug; a malicious relay returns an event whose embedded envelope has `id != <slug>`. If the resolver renders the relay's envelope without re-checking the URL, a sharer's link to one pledge silently displays a different pledge to the recipient. *Mitigated by reference impl*: the hosted `/p/<id>` resolver hard-fails (parse-error panel) when the relay-returned envelope's `id` does not equal the URL slug, even if BIP-322 verifies for the substituted envelope. The same defense applies to share-link fragments (`#<base64url>`): the fragment-decoded envelope is rejected if its `id` differs from the URL. Surfaced during the `oc-pledge-web` resolver build (commit `584de0d`); not anticipated in the original spec but documented here for downstream implementers. *Implementation requirement*: any consumer-built deep-link resolver SHOULD reject relay-returned envelopes whose embedded `id` does not match the URL identifier.

24. **Wrong-target BIP-322 sign-target bug.** A swearer's wallet adapter receives the canonical-message bytes (or any other artifact) instead of the lowercase-hex pledge id, and signs that. SPEC §3.5 requires `sig.value := BIP322(swearer.address, hex(pledge_id))` — any other target produces signatures that no conforming verifier will accept, breaking the swearer's pledge's `kept`-record permanently. *Mitigated by spec*: `verify_pledge` reconstructs the id from the canonical message and runs BIP-322 against `hex(id)`; mis-targeted signatures fail with `E_PLEDGE_BAD_SIG`. *Surfaced during reference-impl build*: this exact bug existed in the pre-migration `oc-pledge-web` composer (signed `preview.canonical` rather than `preview.id`); fixed in commit `584de0d`. *Implementation requirement*: implementers wiring a wallet adapter SHOULD test that re-loading a freshly-signed envelope through `verify_pledge` returns `ok: true` before shipping the integration. If verification fails on a known-good wallet, the most likely explanation is the wrong sign target.
