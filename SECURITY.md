...
Both SDKs cover **SPEC §9.1 steps 1–4 plus step 9** (state classification). They do **not** yet cover, by design, steps 5 and 8:

- **Step 5 — bond live re-resolution against Bitcoin chain state.** Both SDKs ship `verifyBond` / `verify_bond` as a pure algorithm with a caller-injected attestation lookup. The SDK never caches; cache TTL discipline is the caller's responsibility. Consumers using `/verify` or `/p/<id>` on pledge.ochk.io currently surface bond verification as out-of-scope (see the "envelope-only verification today" notice on those pages); enforcement against live chain state ships with the v0.2 chain accessor.
- **Step 8 — deterministic mechanism re-evaluation against public state.** The seven `resolution.mechanism` grammars are validated at the regex level (`validate_resolution_query`); actual chain / Nostr / HTTP / DNS / Vote evaluation is left to consumers. Same v0.2 cut-line.

Two layers of §9.1 verification therefore live with the consumer until v0.2 of the SDKs:

1. **OC Agent §7.3 delegation lookup + scope enforcement.** When `via_delegation` is present in a pledge envelope, both SDKs check the envelope shape and verify BIP-322 against `agent_address` per §7.3 step 6, but do **not** perform §7.3 steps 1–5 (resolve the delegation, confirm `principal == swearer`, confirm the `pledge:create` scope constraints fit the pledge). Consumers attaching consequential weight to agent-delegated pledges MUST layer `@orangecheck/agent-core` resolution on top. Native agent-core integration ships in pledge-core v0.2. See scenario 15.
2. **`http_get_hash` majority-of-three and `dns_record` multi-resolver agreement** (SPEC §3.4.5, §3.4.6). The SDKs validate query shape only; consumers ship the resolver discipline.
...
15. **Agent goes rogue under unscoped delegation.** A principal issues an OC Agent delegation with `pledge:create` scope and no constraints; the agent swears pledges the principal didn't intend. *Mitigated by scope grammar*: SPEC §7.3 supports `max_bond_sats`, `mechanism`, and `counterparty` constraints. Principals SHOULD issue narrowly-scoped delegations. *Residual risk accepted*: a principal who issues an unconstrained delegation accepted the risk; the public ledger reflects that the principal is the swearer of record, with the delegation cited. **Implementation note (2026-05)**: `pledge-core` and `orangecheck.pledge` currently verify the envelope shape and BIP-322 signature against `agent_address` per §7.3 step 6, but do **not** perform §7.3 steps 1–5 (delegation resolution + scope-constraint validation). Consumers attaching consequential weight to agent-delegated pledges MUST resolve the delegation via `@orangecheck/agent-core` and reject pledges whose `bond.min_sats` exceeds the delegation's `max_bond_sats`, or whose `resolution.mechanism` falls outside the delegation's `mechanism=…` constraint. Native agent-core integration is the headline gate for pledge-core v0.2.

16. **Delegation forgery.** An attacker fabricates a delegation that authorizes them as agent for a target principal. *Mitigated*: the delegation is BIP-322-signed by the principal under OC Agent v1's verification rules. Fabricated delegations fail principal-signature check. **Implementation note (2026-05)**: same caveat as scenario 15 — the v0.1 reference SDKs do not currently resolve the delegation. Consumers must run agent-core's `verifyDelegation` separately. The pledge envelope itself is sound (the agent_address claim is unsigned, but the BIP-322 sig.value verifies under agent_address — so a fabricated agent_address would also need a matching agent private key to produce a valid sig). The forgery vector exists only if a consumer skips the delegation lookup.
